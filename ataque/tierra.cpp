...
void Tierra :: atacar(Personaje** personajeAtacado ){
    for(int i = 0; i < MAXRIVALES ; i++){
        if(personajeAtacado[i] != nullptr){
            int filaAtacar = personajeAtacado[i]->obtenerFila();
            int columnaAtacar = personajeAtacado[i]->obtenerColumna();
            int danoRango = danoPorRango(fila,columna,filaAtacar,columnaAtacar);
